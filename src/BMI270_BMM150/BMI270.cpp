...
void BoschSensorClass::Rev2_debug(Stream& stream)
{
  _debug = &stream;
}
#ifdef __MBED__
void BoschSensorClass::Rev2_onInterrupt(mbed::Callback<void(void)> cb)
{
  if (BMI270_INT1 == NC) {
    return;
  }
...
#endif
int BoschSensorClass::Rev2_begin() {
  ...
  Rev2_accelOffset[3] = {0.0f};
  Rev2_gyroOffset[3] = {0.0f};
...
void BoschSensorClass::Rev2_setContinuousMode() {
  bmi2_set_fifo_config(BMI2_FIFO_GYR_EN | BMI2_FIFO_ACC_EN, 1, &bmi2);
  continuousMode = true;
}

void BoschSensorClass::Rev2_oneShotMode() {
...
*/
int BoschSensorClass::Rev2_readAcceleration(float& x, float& y, float& z) {
  ...
  #ifdef TARGET_ARDUINO_NANO33BLE
  x = Rev2_accelOffset[0] + (-sensor_data.acc.y / conversionFactor);
  y = Rev2_accelOffset[1] + (-sensor_data.acc.x / conversionFactor);
  #else
  x = Rev2_accelOffset[0] + (sensor_data.acc.x / conversionFactor);
  y = Rev2_accelOffset[1] + (sensor_data.acc.y / conversionFactor);
  #endif
  z = Rev2_accelOffset[2] + (sensor_data.acc.z / conversionFactor);
  return (ret == 0);
}
...
*/
void BoschSensorClass::Rev2_setAccelOffset(float offset[]) {
  Rev2_accelOffset[0] = offset[0] / 9.81;
  Rev2_accelOffset[1] = offset[1] / 9.81;
  Rev2_accelOffset[2] = offset[2] / 9.81;
}

/**
* returns the offset value of the accel.
*/
float* BoschSensorClass::Rev2_getAccelOffset() {
  return Rev2_accelOffset;
}

/**
 * Checks the status of the acceleration sensor. Returns 0/ false, if sensor is not ready
*/
int BoschSensorClass::Rev2_accelerationAvailable() {
  uint16_t status;
...
*/
float BoschSensorClass::Rev2_getAccelODR() {
  struct bmi2_sens_config sens_cfg;
  sens_cfg.type = BMI2_ACCEL;
  bmi2_get_sensor_config(&sens_cfg, 1, &bmi2);
...
*/
int BoschSensorClass::Rev2_setAccelODR(int8_t setting) { 
...
*/
int BoschSensorClass::Rev2_setAccelFS(int8_t setting) { 
...
*/
float BoschSensorClass::Rev2_getAccelFS() {
  struct bmi2_sens_config sens_cfg;
...
*/
int BoschSensorClass::Rev2_readGyroscope(float& x, float& y, float& z) {
  ...
  #ifdef TARGET_ARDUINO_NANO33BLE
  x = Rev2_gyroOffset[0] + (-sensor_data.gyr.y / conversionFactor);
  y = Rev2_gyroOffset[1] + (-sensor_data.gyr.x / conversionFactor);
  #else
  x = Rev2_gyroOffset[0] + (sensor_data.gyr.x / conversionFactor);
  y = Rev2_gyroOffset[1] + (sensor_data.gyr.y / conversionFactor);
  #endif
  z = Rev2_gyroOffset[2] + (sensor_data.gyr.z / conversionFactor);
  return (ret == 0);
}
...
*/
void BoschSensorClass::Rev2_setGyroOffset(float offset[]) {
  Rev2_gyroOffset[0] = offset[0];
  Rev2_gyroOffset[1] = offset[1];
  Rev2_gyroOffset[2] = offset[2];
}

float* BoschSensorClass::Rev2_getGyroOffset(){
  return Rev2_gyroOffset;
}

/**
 * checks the status of the gyroscope. Returns 0/false, if sensor is not ready
*/
int BoschSensorClass::Rev2_gyroscopeAvailable() {
  uint16_t status;
  bmi2_get_int_status(&status, &bmi2);
...
*/
float BoschSensorClass::Rev2_getGyroODR() {
  struct bmi2_sens_config sens_cfg;
  sens_cfg.type = BMI2_GYRO;
  bmi2_get_sensor_config(&sens_cfg, 1, &bmi2);
...
* every other setting results in ODR = 100Hz
*/
int BoschSensorClass::Rev2_setGyroODR(int8_t setting) { 
...
*/
int BoschSensorClass::Rev2_setGyroFS(int8_t setting) { 
...
*/
float BoschSensorClass::Rev2_getGyroFS() {
...
// Magnetometer
int BoschSensorClass::Rev2_readMagneticField(float& x, float& y, float& z) {
...
int BoschSensorClass::Rev2_magneticFieldAvailable() {
  bmm150_get_interrupt_status(&bmm1);
  return bmm1.int_status & BMM150_INT_ASSERTED_DRDY;
}

float BoschSensorClass::Rev2_getMagnetODR() {
...
*/
int BoschSensorClass::Rev2_setMagnetODR(int8_t setting) {
